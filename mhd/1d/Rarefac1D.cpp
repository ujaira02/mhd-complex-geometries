// ============================================================================
// Rarefac1D.cpp — 1D rarefaction wave with an out-of-plane magnetic field
// ----------------------------------------------------------------------------
//   - HLLD fluxes
...
//
// Initial conditions:
//   L: ρ=1.7  u=0  v=0  w=0 Bx=1.1
//      By=1.0            Bz=0.0            p=1.7
//   R: ρ=0.2  u=0  v=0  w=-1.496891  Bx=1.1
//      By=0.78588731621  Bz=0.61836983763  p=0.2
//   Discontinuity: x0=0.4  γ = 5/3  t_out=0.15
// ============================================================================
#include "Rarefac1D.h"
...
static bool prompt_use_glm() {
    std::cout << "[rarefaction] Enable hyperbolic divergence cleaning (GLM)? [y/n, default y]: ";
    std::string answer;
...
static void write_approx_dat_csv(const std::string& baseDir, int N,
    ...
    std::cerr << "[rarefaction] Wrote " << datPath << "\n";
    std::cerr << "[rarefaction] Wrote " << csvPath << "\n";
}
...
static void write_overlay_plots(const std::string& baseDir, double L) {
    ...
    for (const auto& v : vars) {
        const std::string pltPath = pltDir + "/Rarefac1D_" + v.name + ".plt";
        std::ofstream gp(pltPath);
        gp << "set term pngcairo size 900,650\n";
        gp << "set output '" << pngDir << "/Rarefac1D_" << v.name << ".png'\n";
        gp << "unset grid\n";
...
static void write_cut_and_plot(const std::string& base,
    ...
    {
        std::ofstream f(base + "/data/Rarefaction1D_Multiplot.dat");
        f << "# x  rho  p  vx  vz  By  Bz\n";
    ...
    {
        std::ofstream gp(base + "/plots/plt/Rarefaction1D_Multiplot.plt");
        gp << "set term pngcairo size 1000,900\n";
        gp << "set output '" << base << "/plots/png/Rarefaction1D_Multiplot.png'\n";
        gp << "unset grid\n";
        gp << "set border linewidth 1.2\n";
        gp << "set tics scale 0.6\n";
        gp << "set multiplot layout 3,2 rowsfirst title 'Rarefaction Wave'\n";
        gp << "set xlabel 'x'\n";
        gp << "set xrange [0:1]\n";
        gp << "unset key\n";
        gp << "set arrow from 0.4,graph 0 to 0.4,graph 1 nohead lc rgb '#aaaaaa' lw 1 dt 2\n";
        gp << "set title '{/Symbol r}'\n";
        gp << "set yrange [*:*]\n";
        gp << "plot '" << base << "/data/Rarefaction1D_Multiplot.dat' u 1:2 w p pt 7 ps 0.25 lc rgb '#000000'\n";
        gp << "set title 'p'\n";
        gp << "set yrange [*:*]\n";
        gp << "plot '" << base << "/data/Rarefaction1D_Multiplot.dat' u 1:3 w p pt 7 ps 0.25 lc rgb '#000000'\n";
        gp << "set title 'v_x'\n";
        gp << "set yrange [*:*]\n";
        gp << "plot '" << base << "/data/Rarefaction1D_Multiplot.dat' u 1:4 w p pt 7 ps 0.25 lc rgb '#000000'\n";
        gp << "set title 'v_z'\n";
        gp << "set yrange [*:*]\n";
        gp << "plot '" << base << "/data/Rarefaction1D_Multiplot.dat' u 1:5 w p pt 7 ps 0.25 lc rgb '#000000'\n";
        gp << "set title 'B_y'\n";
        gp << "set yrange [*:*]\n";
        gp << "plot '" << base << "/data/Rarefaction1D_Multiplot.dat' u 1:6 w p pt 7 ps 0.25 lc rgb '#000000'\n";
        gp << "set title 'B_z'\n";
        gp << "set yrange [*:*]\n";
        gp << "plot '" << base << "/data/Rarefaction1D_Multiplot.dat' u 1:7 w p pt 7 ps 0.25 lc rgb '#000000'\n";
        gp << "unset multiplot\n";
        gp.close();
    }

    if (cmd_exists("gnuplot")) {
        syscmd("gnuplot \"" + base + "/plots/plt/Rarefaction1D_Multiplot.plt\" >/dev/null 2>&1");
    }
}
...
void run_rarefac() {
    ...
    const double x_discontinuity = 0.4;
    ...
    for (int Nx : resolutions) {
        ...
        const std::string base = "outputs/MHD/1D/Rarefaction";
        write_approx_dat_csv(base, Nx, G.U, G.ng, G.dx);
        if (Nx == 400) {
            write_cut_and_plot(base, s_out, rho_out, p_out, vx_out, vz_out, By_out, Bz_out);
            std::cerr << "[rarefaction] Wrote data to " << base << "/data\n";
            std::cerr << "[rarefaction] Wrote plots to " << base << "/plots/png\n";
        }
