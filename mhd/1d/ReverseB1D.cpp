// ============================================================================
// ReverseB1D.cpp — 1D shock tube with transverse magnetic field reversal
// ----------------------------------------------------------------------------
//   - HLLD fluxes
...
//
// Initial conditions:
//   L: ρ=1.0  u=0  v=0  w=0  Bx=1.3
//      By=1.0   Bz=0.0  p=1.0
//   R: ρ=0.4  u=0  v=0  w=0  Bx=1.3
//      By=-1.0  Bz=0.0  p=0.4
...
static void write_approx_dat_csv(const std::string& baseDir, int N,
    ...
    std::cerr << "[reverse b-field] Wrote " << datPath << "\n";
    std::cerr << "[reverse b-field] Wrote " << csvPath << "\n";
}
...
static void write_overlay_plots(const std::string& baseDir, double L) {
    ...
    for (const auto& v : vars) {
        const std::string pltPath = pltDir + "/BrioWu_" + v.name + ".plt";
        std::ofstream gp(pltPath);
        gp << "set term pngcairo size 900,650\n";
        gp << "set output '" << pngDir << "/BrioWu_" << v.name << ".png'\n";
        gp << "unset grid\n";
...
static void write_cut_and_plot(const std::string& base,
    ...
    {
        std::ofstream f(base + "/data/ReverseB1D_Multiplot.dat");
    ...
    {
        std::ofstream gp(base + "/plots/plt/ReverseB1D_Multiplot.plt");
        gp << "set term pngcairo size 1000,900\n";
        gp << "set output '" << base << "/plots/png/ReverseB1D_Multiplot.png'\n";
        gp << "unset grid\n";
        gp << "set border linewidth 1.2\n";
        gp << "set tics scale 0.6\n";
        gp << "set multiplot layout 3,2 rowsfirst title 'Reverse B-field Wave'\n";
        gp << "set xlabel 'x'\n";
        gp << "set xrange [0:1]\n";
        gp << "unset key\n";
        gp << "set arrow from 0.5,graph 0 to 0.5,graph 1 nohead lc rgb '#aaaaaa' lw 1 dt 2\n";
        ...
        gp << "set yrange [*:*]\n";
        gp << "plot '" << base << "/data/ReverseB1D_Multiplot.dat' u 1:2 w p pt 7 ps 0.25 lc rgb '#000000'\n";
        gp << "set title 'p'\n";
        gp << "set yrange [*:*]\n";
        gp << "plot '" << base << "/data/ReverseB1D_Multiplot.dat' u 1:3 w p pt 7 ps 0.25 lc rgb '#000000'\n";
        gp << "set title 'v_x'\n";
        gp << "set yrange [*:*]\n";
        gp << "plot '" << base << "/data/ReverseB1D_Multiplot.dat' u 1:4 w p pt 7 ps 0.25 lc rgb '#000000'\n";
        gp << "set title 'v_y'\n";
        gp << "set yrange [*:*]\n";
        gp << "plot '" << base << "/data/ReverseB1D_Multiplot.dat' u 1:5 w p pt 7 ps 0.25 lc rgb '#000000'\n";
        gp << "set title 'B_y'\n";
        gp << "set yrange [*:*]\n";
        gp << "plot '" << base << "/data/ReverseB1D_Multiplot.dat' u 1:6 w p pt 7 ps 0.25 lc rgb '#000000'\n";
        gp << "set title 'B_z'\n";
        gp << "set yrange [*:*]\n";
        gp << "plot '" << base << "/data/ReverseB1D_Multiplot.dat' u 1:7 w p pt 7 ps 0.25 lc rgb '#000000'\n";
        gp << "unset multiplot\n";
        gp.close();
    }

    if (cmd_exists("gnuplot")) {
        syscmd("gnuplot \"" + base + "/plots/plt/ReverseB1D_Multiplot.plt\" >/dev/null 2>&1");
    }
}
...
void run_reverse_b() {
    ...
    for (int Nx : resolutions) {
        ...
        const std::string base = "outputs/MHD/1D/ReverseB";
