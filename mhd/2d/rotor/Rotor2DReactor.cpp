// ============================================================================
// Rotor2DReactor.cpp — MHD rotor (2D) with SDF-based reflective boundary
// ----------------------------------------------------------------------------
//   - HLLD fluxes
...
// Initial conditions:
//   (0.0, 0.0), r0 = 0.1, r1 = 0.115, with r = √(x² + y²)
...
static constexpr double y_min = -1;
static constexpr double y_max = 1;
static constexpr double SDF_SCALE = 1.05;
static constexpr double rotor_center_x = -0.10;
static double rotor_center_y = 0.0;
...
static const double Bx0 = 0;
static const double By0 = B_mag * 2.5;

struct SdfTableRotor2D {
    ...
    bool load(const std::string& path) {
        std::ifstream f(path);
        if (!f.is_open()){ std::cerr << "[rotor] Cannot open: "<<path<<"\n"; return false; }
        ...
        if (xs.empty()) { 
            std::cerr << "[rotor] No data in: " << path << "\n";
            return false;
        }
        ...
        if (nx * ny != static_cast<int> (xs.size())) {
            std::cerr << "[rotor] Size " << xs.size() << " not divisible by nx = " << nx << "\n";
            return false;
        }
        ...
        std::cerr << "[rotor] Normalised: x[" << xmin << "," << xmax << "]  y[" << ymin << "," << ymax << "]\n";
    ...
    void gradient(double x, double y, double& gx, double& gy) const {
        const double ex = dx;
        const double ey = dy;
        gx = (interp(x + ex, y) - interp(x - ex, y)) / (2.0 * ex);
        gy = (interp(x, y + ey) - interp(x, y - ey)) / (2.0 * ey);
...
void run_rotor_reactor() {
    ...
    std::cout << "\n[rotor] Rotation angle theta (degrees) [default 0]: ";
    std::string ts;
    ...
    if (!ts.empty()) {
        try {
            ...
            std::cerr << "[rotor] Invalid rotation angle.\n";
        }
    }
    set_box_rotation_angle(theta);
    std::cerr << "[rotor] Rotation theta = " << theta << " deg\n";

    const bool use_glm = prompt_use_glm();

    std::cout << "\n[rotor] Resolution N (cells per dimension, N² total) [default 256]: ";
    std::string ns;
    ...
    std::cerr << "[rotor] N² = " << N << "² (" << (long long) N * N << " cells)\n";

    const std::vector<double> t_outs = {0.0, 0.01, 0.1, 0.25, 0.5, 0.65};
    const std::vector<std::string> t_tags = {"t000", "t001", "t010", "t025", "t050", "t065"};
    ...
    for (int j = 0; j < G.ny(); ++j) {
        for(int i = 0; i < G.nx(); ++i) {
            ...
            const double u_r = -10.0 * dy;
            const double v_r = 10.0 * dx;
    ...
    const std::string base = "outputs/MHD/2D/Rotor/Reflective/" + run_label;
    mkdir_p(base + "/data");
    mkdir_p(base + "/plots/plt");
    mkdir_p(base + "/plots/png");
