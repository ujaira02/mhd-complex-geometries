// ============================================================================
// Rotor2DCirc.cpp — MHD rotor (2D) with circular reflective boundary
// ----------------------------------------------------------------------------
//   - HLLD fluxes
...
static constexpr double GLM_ALPHA = 0.18;
...
static void damp_glm_psi(Grid& G, double dt, double ch, bool use_glm) {
    ...
    const int ng = G.ng, nx = G.nx(), ny = G.ny();
    const double rate = ch * GLM_ALPHA / std::max(std::min(G.dx, G.dy), 1e-14);
...
static void dump_fields(const Grid& G, const std::string& base, double tout, const std::string& tag) {
    ...
    make_colour_plot(base, "pmag_" + tag + ".dat", "pmag_" + tag + ".png",
                     "Rotor p_box at t = " + std::to_string(tout) + "s", "p_B");
...
void run_rotor_circ() {
    ...
    const std::vector<double> t_outs = {0.0, 0.01, 0.1, 0.25, 0.5, 1.0};
    const std::vector<std::string> t_tags = {"t000", "t001", "t010", "t025", "t050", "t100"};
    ...
    const std::string base = "outputs/MHD/2D/Rotor/Reflective/Circular";
    mkdir_p(base + "/data");
    mkdir_p(base + "/plots/plt");
    mkdir_p(base + "/plots/png");
    ...
    while (t < tfinal - 1e-14) {
        ...
        if (!is_finite(dt) || dt <= 0.0) {
            std::cerr << "[rotor] invalid dt=" << dt << " at t=" << t << " step=" << step << "\n";
            break;
        }
